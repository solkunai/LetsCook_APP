...
pub fn create_2022_token<'a>(
    ...
    let token_metadata = spl_token_metadata_interface::state::TokenMetadata {
        ...
        update_authority: OptionalNonZeroPubkey::from(*pda_account.key),
    ...
    let ix = solana_program::system_instruction::create_account(funding_account.key, mint_account.key, mint_rent, space as u64, token_program.key);
    ...
    invoke_signed(
        &spl_token_metadata_interface::instruction::update_authority(
            ...
            mint_account.key,
            pda_account.key,
            OptionalNonZeroPubkey::from(Pubkey::from([0 as u8; 32])),
        ),
        &[mint_account.clone(), pda_account.clone()],
        &[&[&token_config.pda.to_le_bytes(), &[pda_bump]]],
    )?;
