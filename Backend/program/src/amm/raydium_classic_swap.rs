...
use crate::instruction::accounts::SwapRaydiumClassicAccounts;
use crate::utils;
use crate::{
    accounts,
    amm::{get_amm_plugin_map, AMMPlugin, AMMPluginType, TradeToEarn, AMM},
    instruction::RaydiumSwapArgs,
};
use crate::{amm::reward_schedule, state};

#[derive(BorshSerialize, BorshDeserialize, Debug, Clone, PartialEq)]
struct RaydiumClassicSwap {
    pub instruction: u8,
    pub in_amount: u64,
    pub out_amount: u64,
}

pub fn raydium_classic_swap<'a>(program_id: &Pubkey, accounts: &'a [AccountInfo<'a>], args: RaydiumSwapArgs) -> ProgramResult {
    msg!("in classic swap, getting accounts");
    let ctx: crate::instruction::accounts::Context<SwapRaydiumClassicAccounts> = SwapRaydiumClassicAccounts::context(accounts)?;

    utils::check_and_create_ata(
        ctx.accounts.user,
        ctx.accounts.user,
        ctx.accounts.in_token_mint,
    ...
    utils::check_and_create_ata(
        ctx.accounts.user,
        ctx.accounts.user,
        ctx.accounts.out_token_mint,
        ctx.accounts.user_out,
        ctx.accounts.token_program,
    )?;

    let account_metas = vec![
        instruction::AccountMeta::new_readonly(*ctx.accounts.token_program.key, false),
        instruction::AccountMeta::new(*ctx.accounts.pool_id.key, false),
        instruction::AccountMeta::new_readonly(*ctx.accounts.pool_authority.key, false),
        instruction::AccountMeta::new(*ctx.accounts.open_orders.key, false),
        instruction::AccountMeta::new(*ctx.accounts.target_orders.key, false),
        instruction::AccountMeta::new(*ctx.accounts.pool_base.key, false),
        instruction::AccountMeta::new(*ctx.accounts.pool_quote.key, false),
        instruction::AccountMeta::new_readonly(*ctx.accounts.market_program.key, false),
        instruction::AccountMeta::new(*ctx.accounts.market_id.key, false),
        instruction::AccountMeta::new(*ctx.accounts.bids.key, false),
        instruction::AccountMeta::new(*ctx.accounts.asks.key, false),
        instruction::AccountMeta::new(*ctx.accounts.event_queue.key, false),
        instruction::AccountMeta::new(*ctx.accounts.market_base.key, false),
        instruction::AccountMeta::new(*ctx.accounts.market_quote.key, false),
        instruction::AccountMeta::new(*ctx.accounts.market_authority.key, false),
        instruction::AccountMeta::new(*ctx.accounts.user_in.key, false),
        instruction::AccountMeta::new(*ctx.accounts.user_out.key, false),
        instruction::AccountMeta::new(*ctx.accounts.user.key, true),
    ];

    let new_args = RaydiumClassicSwap {
        instruction: if args.side == 0 { 9 } else { 9 },
    ...
    invoke(
        ...
        &[
            ...
            ctx.accounts.pool_base.clone(),
            ctx.accounts.pool_quote.clone(),
            ctx.accounts.market_program.clone(),
            ctx.accounts.market_id.clone(),
            ctx.accounts.bids.clone(),
            ctx.accounts.asks.clone(),
            ctx.accounts.event_queue.clone(),
            ctx.accounts.market_base.clone(),
            ctx.accounts.market_quote.clone(),
            ctx.accounts.market_authority.clone(),
            ctx.accounts.user_in.clone(),
            ctx.accounts.user_out.clone(),
