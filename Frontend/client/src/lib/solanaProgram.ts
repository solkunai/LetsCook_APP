...
// Cache for discriminators to avoid regenerating them
const discriminatorCache = new Map<string, Buffer>();

// Function to get discriminator (with caching)
async function getCachedDiscriminator(name: string): Promise<Buffer> {
  if (!discriminatorCache.has(name)) {
    const discriminator = await getDiscriminator(name);
    discriminatorCache.set(name, discriminator);
  }
  return discriminatorCache.get(name)!;
}

// For now, let's use placeholder discriminators that match common patterns
// These should be replaced with actual discriminators from your deployed program
const INSTRUCTION_DISCRIMINATORS = {
  CREATE_LAUNCH: Buffer.from([0x8a, 0x4c, 0x4e, 0x4a, 0x8b, 0x4d, 0x4f, 0x4b]), // Placeholder
  BUY_TICKETS: Buffer.from([0x9a, 0x5c, 0x5e, 0x5a, 0x9b, 0x5d, 0x5f, 0x5b]), // Placeholder
  JOIN_LAUNCH: Buffer.from([0x9a, 0x5c, 0x5e, 0x5a, 0x9b, 0x5d, 0x5f, 0x5b]), // Same as BuyTickets
  SWAP_COOK_AMM: Buffer.from([0xaa, 0x6c, 0x6e, 0x6a, 0xab, 0x6d, 0x6f, 0x6b]), // Placeholder
  HYPE_VOTE: Buffer.from([0xba, 0x7c, 0x7e, 0x7a, 0xbb, 0x7d, 0x7f, 0x7b]), // Placeholder
  CLAIM_TOKENS: Buffer.from([0xca, 0x8c, 0x8e, 0x8a, 0xcb, 0x8d, 0x8f, 0x8b]), // Placeholder
  EDIT_LAUNCH: Buffer.from([0xda, 0x9c, 0x9e, 0x9a, 0xdb, 0x9d, 0x9f, 0x9b]), // Placeholder
} as const;
...
export async function buildCreateLaunchTransaction(
  ...
  const instructionData = Buffer.alloc(500); // Increased buffer size
  let offset = 0;
  
  // Add instruction discriminator (real discriminator from Web Crypto API)
  const discriminator = await getCachedDiscriminator("create_launch");
  instructionData.set(discriminator, offset);
  offset += 8;
  
  // Serialize all the required fields in the correct order
  // Based on your Rust struct, we need to serialize CreateLaunchArgs
  ...
  instructionData.writeUInt32LE(iconBytes.length, offset);
  offset += 4;
  instructionData.set(iconBytes, offset);
  offset += iconBytes.length;
  
  // banner: String
  const bannerBytes = Buffer.from(args.banner, 'utf8');
  instructionData.writeUInt32LE(bannerBytes.length, offset);
  offset += 4;
  instructionData.set(bannerBytes, offset);
  offset += bannerBytes.length;
  
  // totalSupply: u64
  instructionData.writeBigUInt64LE(BigInt(args.totalSupply), offset);
  offset += 8;
  
  // decimals: u8
  instructionData.writeUInt8(args.decimals, offset);
  offset += 1;
  
  // launchDate: u64
  instructionData.writeBigUInt64LE(BigInt(args.launchDate), offset);
  offset += 8;
  
  // closeDate: u64
  instructionData.writeBigUInt64LE(BigInt(args.closeDate), offset);
  offset += 8;
  ...
  // ticketPrice: u64
  instructionData.writeBigUInt64LE(BigInt(args.ticketPrice), offset);
  offset += 8;
  
  // pageName: String
  const pageNameBytes = Buffer.from(args.pageName, 'utf8');
  instructionData.writeUInt32LE(pageNameBytes.length, offset);
  offset += 4;
  instructionData.set(pageNameBytes, offset);
  offset += pageNameBytes.length;
  
  // transferFee: u32
  instructionData.writeUInt32LE(args.transferFee, offset);
  offset += 4;
  
  // maxTransferFee: u32
  instructionData.writeUInt32LE(args.maxTransferFee, offset);
  offset += 4;
  
  // extensions: u32
  instructionData.writeUInt32LE(args.extensions, offset);
  offset += 4;
  
  // ammProvider: u8
  instructionData.writeUInt8(args.ammProvider, offset);
  offset += 1;
  
  // launchType: u8
  instructionData.writeUInt8(args.launchType, offset);
  offset += 1;
  
  // whitelistTokens: u32
  instructionData.writeUInt32LE(args.whitelistTokens, offset);
  offset += 4;
  
  // whitelistEnd: u64
  instructionData.writeBigUInt64LE(BigInt(args.whitelistEnd), offset);
  offset += 8;
  ...
  console.log('🔍 Transaction validation:', {
    ...
    programId: PROGRAM_ID.toString(),
    instructionDataLength: instructionData.slice(0, offset).length,
